...
module tb_input_controller;
    ...
    logic [31:0] SPI_d;
    logic [15:0] HLBrdata;              // packed 4x4-bit (element k = HLBrdata[4*k +: 4])
    logic Id;
    logic signed [31:0] MAC_in;         // packed 4x8-bit (element k = MAC_in[8*k +: 8])
    ...
    function automatic logic [7:0] expect_byte(input int j, input logic itype);
        logic [3:0] hi;
        hi = SPI_d[31-8*j -: 4];
        if (itype) expect_byte = {hi, HLBrdata[4*(3-j) +: 4]};
    ...
    task automatic transaction(input logic itype, input logic [31:0] spidat);
        ...
        for (int j = 0; j < 4; j++) begin
            ...
            checks++;
            if (MAC_in[8*j +: 8] !== exp) begin
                errors++;
                $display("FAIL: MAC_in[%0d] = %h exp %h (itype=%b)", j, MAC_in[8*j +: 8], exp, itype);
            end else $display("PASS: MAC_in[%0d] = %h", j, MAC_in[8*j +: 8]);
        end
    ...
    initial begin
        ...
        HLBrdata[4*0 +: 4] = 4'h7; HLBrdata[4*1 +: 4] = 4'h6;
        HLBrdata[4*2 +: 4] = 4'h5; HLBrdata[4*3 +: 4] = 4'h4;
